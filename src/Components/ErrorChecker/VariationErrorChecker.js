...
const checkVariableError = function (allData, thisData, header) {
    let skuLocation, parentLocation, attributeValueLocations = []
    for (var c = 0; c < header.length; c++) {
        ...
        else if (String(header[c]).trim().replace(/[0-9]/g, '').toLowerCase() === "attribute  value(s)") {
            attributeValueLocations.push(c);
        }
        // if(skuLocation && parentLocation) break;
    }

    if (!skuLocation || !parentLocation || attributeValueLocations.length === 0) {
        return {
            error: true,
            errorType: "Could not find either SKU or Parent location or Attribute Values Header"
        }
    }
    ...
    for (c = 0; c < attributeValueLocations.length; c++) {
        const brokenAttributeValues = thisData[attributeValueLocations[c]].split(",");
        if (brokenAttributeValues.length > 1) return { error: false }
        if (c === attributeValueLocations.length - 1) return { error: true, errorType: "Varying attribute not found" }
    }
